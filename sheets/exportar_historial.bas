...
Private Sub borrar_titulos()
    Dim sh As Worksheet
    Set sh = ThisWorkbook.Sheets("Historial")
...
Private Sub agregar_titulos()
    ...
    data = Array("Id Mov.", "Código", "Tipo", "Fecha", "Descripción")
...
Private Sub crear_tabla()
    ...
    sh.ListObjects.Add(xlSrcRange, Range("$A$1:$E$" & lRow), , xlYes).Name = _
        "Tabla1"
...
Sub decorar_tabla()
    ...
    Dim rng As Range
    Set rng = sh.Columns("A:D")
    rng.ColumnWidth = 12.91
    Set rng = sh.Columns("C:C")
    rng.ColumnWidth = 17.91
